...
namespace ProtectDc {
  ...
  /*
   * Defintion of one attribute: name,
  ...
  // values to cacracterize an ACG line type
  ...
  // data follows .agc configuration file stucture:
  ...
  public class AgcPatchFile {
    ...
    // class contructor defines Regex objects
    static AgcPatchFile() {
    ...
    //  2 all four out paramters properly filled
  ...
  public class SpgUtil {
    ...
    public static bool CompareReplyField(string requestField, string replyField) {
      ...
      if (requestField == replyField) { // equality is okay
        return true;
      }
      ...
      if (requests.Length == replys.Length) {
        for (int i = 0; i < requests.Length; i++) {
          if (requests[i] == replys[i]) { // equality in equation test is okay
            continue;
          }
